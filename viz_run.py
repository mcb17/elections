...
st.title("Presidential election vote by arrondissement")
st.markdown("<p> The map and the histogram represent the share of vote for the selected candidate by polling station and <br /> arrondissement (ie : votes for the candidate / total votes for the area) </p>", unsafe_allow_html=True)
...
def update_dataframe(data, metric_selected):
    ...
    """
    df = data[['coordinates', 'annee', 'arrondissements', 'id_bvote', 'nb_exprime', metric_selected]]
    ...
    df['part'] = round(df.metric / df.nb_exprime *100, 0) 
...
metric_min = df[['part']].min()[0]
metric_max = df[['part']].max()[0]

@st.experimental_singleton
def color_scale(val, metric_min, metric_max):
...
df['fill_color'] = df['part'].apply(lambda row: color_scale(row, metric_min, metric_max))

if 'fill' not in st.session_state:
    st.session_state['fill'] = df['part'].apply(lambda row: color_scale(row, metric_min, metric_max))
...
with col1:
    ...
    st.markdown("###### Votes by polling station")
    ...
    def map (data): 
        ...
        map_layer = pdk.Layer(
                ...
                get_line_color=[243, 250, 247],
                auto_highlight=True,
                pickable=True,
        )

        tooltip = {"html": "Arrondissements : {arrondissements} <br /> Polling station : {id_bvote} <br /><b> % of votes for this candidate:</b> {part}%"}
...
with col2:
    ...
    st.markdown("### This candidate came in :point_right: : #" + str(int(my_podium))+" place")

    st.markdown("***")

    st.markdown("###### Top 5 : arrondissements that voted the most for this candidate")
    histo = df.groupby(['arrondissements']).agg({'metric':'sum', 'nb_exprime':'sum'})
    histo['part'] = histo.metric / histo.nb_exprime
    histo = histo.sort_values(by='part', ascending=False).head(5)
    histo = histo.reset_index()
    
    fig = px.bar(histo.sort_values(by='part'), x='part',y='arrondissements', text='part', color_discrete_sequence=['rgb(22, 147, 132)'], hover_data={'part':False, 'arrondissements':False})
